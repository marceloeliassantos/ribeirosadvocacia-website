<?php 
    ...
    if(isset($_POST['titulo']))
    {
        if(isset($_POST['outra']))
        {
            ...
            $cat = $cat['id'];
            $noticia->inserir($_POST['titulo'],$_POST['conteudo'],$_SESSION['user'],$cat);
        }
        else
            $noticia->inserir($_POST['titulo'],$_POST['conteudo'],$_SESSION['user'],$_POST['categoria']);
...
<body>
    ...
    <section id="content">
        <div class="container" id="painel">
            ...
            <form class="clear" action="novanoticia.php" method="post" autocomplete="off" role="form">
                <label>Titulo</label><br/><input type="text" name="titulo" required class="form-control"><br/>
                <textarea id="editor" name="conteudo" required></textarea><br/>
                <label>Categoria</label><select name="categoria" <?php if(sizeof($categorias)) echo 'required'; ?>>
                    
                    <?php 
                        foreach($categorias as $categoria):
                            $cat = new Categoria($categoria['id']);
                    ?>
                    <option value="<?php echo $categoria['id']; ?>"><?php echo $cat->get_nome();?></option>
                    <?php endforeach;?>
                
                </select><br/><br/>
                <input type="checkbox" name="checkoutra"/><label>Nova Categoria</label><br/>
                <input type="text" name="outra" id="out" <?php if(!sizeof($categorias)) echo 'required'; else echo 'disabled=true'; ?>><br />
